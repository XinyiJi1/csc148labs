...
def list_internal(t: Tree) -> list:
    ...
    if t.children == []:
        ...
        return sum([list_internal(x) for x in t.children], [])+[t.value]
...
def contains_test_passer(t: Tree, test: Callable[[Any], bool]) -> bool:
    ...
    if t.children == []:
        ...
        return any([contains_test_passer(x, test) for x in t.children]) \
               or test(t.value)
...
def list_if(t: Tree, p: Callable[[Any], bool]) -> list:
    ...
    True
    """
    result = sum([list_if(x, p) for x in t.children], [])
    if p(t.value):
        result += [t.value]
    return result
