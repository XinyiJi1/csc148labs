...
def list_between(node: Union[BTNode, None], start: object, end: object) -> list:
    ...
    if start <= node.data <= end:
        s = [node.data]
    if node.data > start:
        s += list_between(node.left, start, end)
    if node.data < end:
        s += list_between(node.right, start, end)
    return s
...
def concatenate_leaves(t: Union[BTNode, None]) -> str:
    """
    Return the string values in the Tree rooted at t concatenated from left
    to right.
    ...
    >>> t4 = BTNode("four", t3, BTNode("five"))
    >>> concatenate_leaves(t1)
    'one'
    >>> concatenate_leaves(t3)
    'onetwo'
    >>> concatenate_leaves(t4)
    'onetwofive'
    """
    if t is None:
        return ''
...
def sum_internal(t: Union[BTNode, None]) -> int:
    ...
    if t.left is None and t.right is None:
        return 0
    return sum_internal(t.left) + sum_internal(t.right) + t.data
